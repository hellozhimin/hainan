...
//            }
            if (index <channelprevuelist.size()) {
//                System.out.println(channelprevuelist.size()+"==============================1==="+index);
                if(index!=(channelprevuelist.size()-1)){
//                    System.out.println(begintime+"================="+endtime);
                    Map tvodMap1 = (Map) channelprevuelist.get(index+1);
                    nextprevuename =  String.valueOf(tvodMap1.get("prevuename"));
//                    System.out.println("=============111111=================1"+nextprevuename);
                    begintime = String.valueOf(tvodMap1.get("begintime"));
                    endtime = String.valueOf(tvodMap1.get("endtime"));
//                    System.out.println(begintime+"================="+endtime);
                    time222 = begintime.substring(11, 16) + "-" + endtime.substring(11, 16);
                }
...
<body bgcolor="transparent" background="" onLoad="pageInit();" onUnload="pagePlayPause();">
...
<div style="left:0px; top:468px;width:1172px;height:234px; position:absolute;">
    ...
    <div id="currproname"  style="color:#B9DB63;left:140px; top:0px; position:absolute; width:900px; height:28px;font-size:28px;"  ></div>
        ...
        <div id="name" class="font_ellipsis"
             style="left:35px; top:129px; position:absolute; width: 411px; height: 50px;font-size:38px;color:#fff;"
             align="right">
            <%=mixno%>  <%=channelname%>
        </div>
    ...
    <div style="left:250px; top:42px; position:absolute; width: 858px;height: 10px;background-color:#3F434B;" >
        ...
        <%--</div>--%>
        <div style="left:510px; top:109px;width:750px;height:50px; position:absolute;color: #fff;font-size: 30px;"
             class="font_ellipsis">
             当前播放：
        ...
        <div style="left:510px; top:162px;width:750px;height:50px; position:absolute;color: #fff;font-size: 30px;"
             class="font_ellipsis">
    ...
    <div id="procesTimeLogo" style="position:absolute;width:25px;height:20px;left:208px;top:51px;"> <img src="images/tv/flip_bar_arrow.png" alt="" width="25" height="20"></div>
    <div id="speedBarDiv"  style="position:absolute; width:0px; height:10px; left:250px; top:42px;background-color:#00B7F9"></div>
        <div id="speedBarDiv1"
...
function newFFRR(){
    var curTSTVTime= top.TSTVCurrentTime;
    var inputTime=new Date(curTSTVTime);
    var inputMseconds=inputTime.getTime();

    //todo 此处需要对秒数进行处理
    var curTempSeconds=curTSTVTime.getHours()*60*60+curTSTVTime.getMinutes()*60+curTSTVTime.getSeconds();

    var newTempSeconds=inputMseconds+pressFFandRRNum*showtime*1000-8*3600*1000;

    inputTime=new Date(newTempSeconds);

    //拼接出目标时间点
    var targetDate = top.formatStr(inputTime.getFullYear()) +top.formatStr((inputTime.getMonth()+1) )+ top.formatStr(inputTime.getDate()) + "T" + top.formatStr(inputTime.getHours()) +top.formatStr( inputTime.getMinutes() )+"00Z";
    pressFFandRRNum=0;
...
//    }
    type = "FF";
    clearAllTimeout();
    var curTSTVTime= top.TSTVCurrentTime.getTime();
    var endTSTVTime=top.TSTVEndTime.getTime();
    if((curTSTVTime+pressFFandRRNum*showtime)<endTSTVTime){
        pressFFandRRNum++;
    }
//    $("stateImg").src="images/tvod/player_btn_pause.png";
    //显示进度条以及时间
    showProcess();
    //定时器如果没按任何键，则返回播放
//    timeseek=top.mainWin.setTimeout("newFFRR()",1000);
...
//    }
    ...
    clearAllTimeout();
    var curTSTVTime= top.TSTVCurrentTime.getTime();
    var beginTSTVTime= top.TSTVBeginTime.getTime();
    if((curTSTVTime+pressFFandRRNum*showtime)>beginTSTVTime){
        pressFFandRRNum--;
    }
//    $("stateImg").src="images/tvod/player_btn_pause.png";
    //显示进度条以及时间
...
function clearAllTimeout(){
    ...
    if(currpronameTimerId!=null){
        clearTimeout(currpronameTimerId);
        currpronameTimerId=null;
    }
    if(refreshTimerId!=null){
...
//        }
    if (top.currState == 2 || top.currState == 4) {
        top.getTSTVTime();
        //获取当前时移频道的开始、结束、当前时间为毫秒数
        var TSTVPlayTime = top.TSTVCurrentTime.getTime()+pressFFandRRNum*showtime*1000 ;
        var TSBeginTime = top.TSTVBeginTime.getTime();
        var TSEndTime = top.TSTVEndTime.getTime();

        //时间的格式是：Wed Apr 01 01:46:13 GMT+0800 2015
        top.jsDebug("=====swb=====TSCurrentTime==="+TSTVPlayTime);
        top.jsDebug("=====swb=====TSBeginTime===" + TSBeginTime + "====TSEndTime===" + TSEndTime);
        var startTimeStr = formatDate(top.TSTVBeginTime);
        var endTimeStr = formatDate(top.TSTVEndTime);
        top.jsDebug("=====swb=====startTimeStr===" + startTimeStr + "====endTimeStr===" + endTimeStr);


        //按快进键，使得当前时间大于了节目结束的时间，怎跳转回去正常播放节目
        if (TSTVPlayTime > TSEndTime && type == "FF") {
            top.jsDebug("=====swb=====goto end====");
//            clearProcessBarTimer();
//            clearCurrpronameTimer();
//
//            top.doGoToEnd();
...
//            top.jsHideOSD();
        ...
        document.getElementById("speedBarDiv1").style.visibility = "visible";
...
//        getTVCurrtenttimeShow(formatDate(top.TSTVCurrentTime));
        getTVCurrtenttimeShow(formatDate(new Date(TSTVPlayTime)));
...
//        top.jsDebug();
        ...
        var TSBeginTime = top.TSTVBeginTime.getTime();
        var TSEndTime = top.TSTVEndTime.getTime();
        showtime=parseInt((TSEndTime-TSBeginTime)/20/1000);
