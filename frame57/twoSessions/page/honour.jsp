...
<%
	...
	String isFromScorer = request.getParameter("isFromScorer")==null?"0":request.getParameter("isFromScorer");
%>
<html>
<head>
...
<div style="position:absolute; top:0px; left:0px; width:1280px; height:720px; ">
    ...
    <!--C3-->
    <div class="topsign">强中国</div>
    <!--C3_a1-->
    <div style="position:absolute; left:87px; top:106px; width:362px; height:564px;">
    	<!--list_up&down-->
        <img src="images/page_up.png" style="position:absolute; top:-5px; left:147px;">
        <img src="images/page_down.png" style="position:absolute; bottom:-5px; left:147px;">
    	<!--news_list-->
    ...
    <!--C3-a2-->
    <div style="position:absolute; top:174px; left:538px; border-width:3px; border-style:solid; border-color:rgba(11,94,125,0.9); width:666px; height:373px;">
    </div>
...
<script type="text/javascript">
	...
	var cookie_cat_id = "honour_category_id";
	var cookie_focus = "honour";
	...
	function setAreaAttr(){
		...
		area0.okEvent = function(isAuto){
			if(select_focus==area0.curDomIndex && select_page==area0.pageNumber && isAuto==undefined){
				...
				if(content_list.length>0){
					...
					window.location.href = "goto_play.jsp?action=out&returnUrl=honour.jsp&vodId="+content_list[area0.curDomIndex+_offset].vodId+"&columncode="+load_category_id();
				}
				return;
			}
			get_play_info();
		...
		focus_logic.page.backEvent = function(){
			...
			if(<%=isFromScorer%>=="0"){
				...
				window.location.href = "scorer.jsp";
			}
		}
