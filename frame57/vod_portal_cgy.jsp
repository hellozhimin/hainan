...
<epg:PageController name="vod_portal_cgy.jsp"/>
<%
...
<div id="test" style="position:absolute; width:300px; height:200px; color:#FF0000; font-size:26px; left:110px; top:50px; z-index:9999"></div>
<!-- VOD 总数区域 -->
<div id="vodNum"></div>
<!-- VOD海报区域 -->
...
function showPorgramOth(){
    ...
    for (var i = 0; i < programNumCurPage; i++) {
    	//解决EC2016V1盒子卡死的问题，替换成默认图  modify by cny 2017/8/9
    	 if(vcolumncode=="0B0Z" && (stbType.indexOf('EC2106V1') != -1 || stbType.indexOf('HG650') != -1)){
...
function doOK() {
	...
	restFTEPGModel("|portal|cgy|0|" + leefocus);
    if(areaIndex===1){
		if(columnData[columnIndex].subexist == '1'){
		
			
		}else{
			restFTEPGModel("|portal|cgy|1|" + programData[programIndex].programType);
			restFTEPGModel("|portal|cgy|2|" + programData[programIndex].columnId);
			restFTEPGModel("|portal|cgy|3|" + programData[programIndex].programId);
			restFTEPGModel("|portal|cgy|4|" + programData[programIndex].mediaservices);
